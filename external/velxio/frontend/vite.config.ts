...
export default defineConfig(({ command }) => ({
  ...
  server: {
    port: 5174,
    // WIREUP: this instance is embedded in Wireup's /simulation page, which may
    // be served from any origin while this dev server runs on the user's own
    // machine. Vite 6 rejects unrecognised Host headers with a blank page that
    // is invisible inside an iframe, and a restrictive frame-ancestors would
    // break the embed the same way. Both are relaxed here because this server
    // is local-only and holds no session; tighten them for a deployment.
    allowedHosts: true,
    headers: {
