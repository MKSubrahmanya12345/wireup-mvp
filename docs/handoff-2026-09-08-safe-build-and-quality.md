...
npm run verify:firmware               # generate 14 builds, compile-check every sketch
...
| `scripts/compile-check.ts` | `npm run verify:firmware`: 14 representative prompts → generate → static checks → `g++ -fsyntax-only` |
...
npm run verify:firmware              → 14/14 sketches clean (all compile; §5 case list)
